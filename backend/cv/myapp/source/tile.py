...
def main():
    parser = argparse.ArgumentParser(description="Robust MRXS to TIFF Converter")
    parser.add_argument("--input", "-i", required=True, help="Path to input file")
    parser.add_argument("--level", "-l", type=int, default=3, help="Zoom level")
    parser.add_argument("--tile-size", type=int, default=2048)
    parser.add_argument("--threshold", type=int, default=15)
    parser.add_argument("--use-associated", default="auto", choices=["auto", "macro", "label", "thumbnail"])
    ...
    # Wymuszenie nazewnictwa: <orginalna_nazwa>_processed.tiff
    output_filename = f"{input_path.stem}_processed.tiff"
    output_path = input_path.parent / output_filename

    logging.info("="*40)
    logging.info(f"Processing: {input_path.name}")
    logging.info(f"Target:     {output_filename}")
    logging.info("="*40)

    processor = SlideProcessor(
        str(input_path), args.level, args.tile_size, args.threshold, args.use_associated
    )

    result_img = processor.process()

    if result_img:
        # Ostatni szlif - przycięcie
        final_img = processor.crop_content(result_img)
        success = save_result(final_img, str(output_path))
        ...
        logging.error("FAILED: Could not generate any image (all methods failed).")
        logging.error("Try increasing the --level argument (e.g., -l 4).")
        return 1
